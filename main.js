    ...
    if(!goalBuckets.length){
      ...
      const rows = goalBuckets.map(b=>{
        ...
        if(saved>=b.goalAmt){
          ...
          const projDate=new Date(); projDate.setMonth(projDate.getMonth()+monthsNeeded);
          projText=t('goals_projected').replace('{date}',projDate.toLocaleDateString(locale,{month:'long',year:'numeric'}));
        ...
        return '<div class="rep-row" style="flex-direction:column;align-items:stretch;gap:5px;padding:10px 0">'+
          '<div style="display:flex;justify-content:space-between;font-size:13px;font-weight:600;color:var(--sage-d)"><span>'+b.name+'</span><span>'+fmt(Math.max(0,saved))+' / '+fmt(b.goalAmt)+'</span></div>'+
          '<div style="height:6px;background:var(--border);border-radius:3px;overflow:hidden"><div style="height:100%;width:'+pct+'%;background:var(--sage)"></div></div>'+
...
function previewInvoice(){
  ...
  document.getElementById('inv-preview').innerHTML=`
    ...
    <div class="inv-totals">
      ...
      ${premType==='yes'?`<div class="inv-total-row"><span>${t('inv_premium_row').replace('{pct}',premPct)}</span><span>${fmt2(premAmt)}</span></div>`:''}
      ${taxType==='yes'?`<div class="inv-total-row"><span>${t('inv_taxrow').replace('{pct}',taxPct)}${taxReg?t('inv_tax_reg_suffix').replace('{reg}',taxReg):''}</span><span>${fmt2(taxAmt)}</span></div>`:''}
