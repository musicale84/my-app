...
function renderTrashList(){
  ...
  el.innerHTML = list.slice().sort((a,b)=>new Date(b.deletedAt)-new Date(a.deletedAt)).map(g=>{
    ...
    return '<div class="trash-row"><div class="trash-info"><div class="trash-name">'+g.name+'</div>'+
      '<div class="trash-meta">'+fmtD(g.date)+' · '+t('set_trash_days_left').replace('{days}',daysLeft)+'</div></div>'+
...
function applyCloudDataLocally(cloudBlob, cloudGigsRows){
  ...
  S.gigs = cloudGigsRows.filter(r=>!r.deleted_at).map(r=>r.data);
  S.deletedGigs = cloudGigsRows.filter(r=>r.deleted_at).map(r=>Object.assign({}, r.data, {deletedAt:r.deleted_at}));
  applyingRemoteChange = false;
  save();
}
async function runMigrationCombine(cloudBlob, cloudGigsRows){
  ...
  const cloudGigs = cloudGigsRows.filter(r=>!r.deleted_at).map(r=>r.data);
  const cloudDeleted = cloudGigsRows.filter(r=>r.deleted_at).map(r=>Object.assign({}, r.data, {deletedAt:r.deleted_at}));
...
function deleteGig(id){
  const g = S.gigs.find(x=>x.id===id); if(!g) return;
  const confirmed = confirm(t('gig_delete_confirm').replace('{name}',g.name).replace('{date}',fmtD(g.date)));
  if(!confirmed) return;
