...
// S.tipsSeen, S.lastGigSort, S.lastRepSort, S.deletedGigs (all purely device-local bookkeeping/UI
// state — syncing them would just add network chatter with no real user-facing value).
const SYNCED_BLOB_KEYS = [
  'onboarded','checkinAnchorDate','moneyDialTags','customDialTags','investPendingConfirm',
  'hasSetLanguage','lang','invoiceLogo','settings','balances','debt','selfLoan','currency',
...
// local-only field (tipsSeen, analytics, sort prefs, etc. — see SYNCED_BLOB_KEYS). Without this
// dedup check, that would still schedule a real network push of a blob that's byte-identical to
// what's already synced, and briefly flip the status badge to "syncing" for no visible reason.
