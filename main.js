...
const SYNCED_BLOB_KEYS = [
  ...
  'hasSetLanguage','lang','invoiceLogo','invoices','settings','balances','debt','selfLoan','selfLoanBaseline','currency','bizExpenses',
...
function calcLoanEntry(fee, cart, date, excludeId, moveToSetAside, taxable, countsTowardBaseline){
  ...
  const baselineGap = Math.max(0, s.baseline - monthNetSoFar);
  ...
  return {
    ...
    gigSurplus:0, baselineGap, baselineCovered: monthNetSoFar>=s.baseline, countsTowardBaseline: counts,
...
function saveGig(){
  ...
  if(editId){
    ...
    if(old){
      ...
      // A Loan being edited already added its old amount as a tracked obligation — reverse it before
      // adding the new amount below, same reversal pattern as everything else in an edit.
      if(old.type==='Loan' && old.fee>0) S.debt.loan = Math.max(0,(S.debt.loan||0)-old.fee);
    }
  ...
  // A Loan is a real, tracked obligation the moment it's received — the same debt ledger everything
  // else (loan-pay-off from a gig's surplus, Settings' own debt balance) already reads from.
  if(isLoan && fee>0) S.debt.loan = (S.debt.loan||0) + fee;
...
function updateDash(){
  ...
  renderBaselineTopupBanner(mInc, s.baseline);
