...
let S={
  ...
  checkinInProgress:false, checkinResumeStep:0, inactivityNudge:'sometimes', lastInactivityNudgeKey:null,
...
function resumeCheckin(){
  ...
  checkinStep=Math.max(0,Math.min(step,3));
  renderCheckinStep();
...
function computeInactivityNudge(){
  ...
  const mInc = S.gigs.filter(g=>g.status==='Received'&&g.receivedDate&&(()=>{const d=new Date(g.receivedDate+'T12:00:00');return d.getFullYear()===y&&d.getMonth()===m;})()).reduce((t,g)=>t+g.netLiquid,0);
  const baseline = S.settings.baseline||0;
...
function renderCheckinStep(){
  ...
  const checkinSteps=checkinIncludesFollowup?[0,1,2,3]:[1,2,3];
...
const SYNCED_BLOB_KEYS = [
  ...
  'checkinInProgress','checkinResumeStep','inactivityNudge','lastInactivityNudgeKey',
  'hasSetLanguage','lang','invoiceLogo','invoices','settings','balances','debt','selfLoan','currency',
...
function saveGig(){
  ...
  const r = calcSurplus(fee, cart, type, applyHst, applyDues, baselineDate, editId, selfLoanBorrowReq, selfLoanRepayReq, ccPayReq, loanPayReq);
  // Flag entries where the scale fee was derived via the check-amount reverse-calculator, plus the
  // original check amount — a visible record on the entry's detail view of how the number came about,
  // for the user's own trust/audit purposes. Reuses g-fee's existing autofilled marker (set by
  // reverseCalcFee()) rather than tracking this a second way.
  const checkAmtVal = parseFloat(document.getElementById('g-check-amt')?.value)||0;
  const feeFromCheckAmount = document.getElementById('g-fee')?.dataset.autofilled==='1' && checkAmtVal>0;
  const gig = {
    id: editId || Date.now(), entryOrder: editId ? (S.gigs.find(g=>g.id===editId)?.entryOrder || Date.now()) : Date.now(),
    ...
    feeFromCheckAmount, checkAmountValue: feeFromCheckAmount ? checkAmtVal : null
  };
  if(editId){
...
function updateDash(){
  ...
  renderInactivityNudgeBanner();
  const rec=S.gigs.filter(g=>g.status==='Received');
  const mG=rec.filter(g=>{const d=new Date(g.receivedDate+'T12:00:00');return d.getMonth()===m&&d.getFullYear()===y;});
...
function getMonthlyHistory(){
  ...
  while(cursor<=end){
    ...
    const mg = rec.filter(g=>{const d=new Date(g.receivedDate+'T12:00:00');return d.getFullYear()===y&&d.getMonth()===m;});
...
function invoiceRow(inv){
  ...
  return '<div class="gig-row" onclick="openInvoiceForEditing('+inv.id+')" role="button" tabindex="0" aria-label="'+invoiceDraftName(inv)+'">'+
    ...
    '<button onclick="event.stopPropagation();deleteInvoiceRecord('+inv.id+')" aria-label="Delete" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:16px;padding:4px;margin-left:6px"><i class="ti ti-trash" aria-hidden="true"></i></button>'+
    '</div></div>';
}
...
function renderInvLines(){
  ...
  el.innerHTML=itemLines.map(l=>`
    <div style="border:1.5px solid var(--border);border-radius:var(--rs);padding:10px;margin-bottom:8px">
      <div style="display:flex;gap:7px;margin-bottom:${l.mode==='flat'?'0':'7px'}">
