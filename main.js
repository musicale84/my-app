...
let S={
  ...
  devPremiumUnlocked:false, invoiceLogo:null,
  tipsSeen:{},
...
en:{
  ...
  rep_excel_locked_note:"Excel export is a premium feature — enable it in Settings → Developer / Testing.", rep_export_field:"Field", rep_export_value:"Value", rep_export_goal:"Goal", rep_export_saved:"Saved", rep_export_target:"Target", rep_export_status:"Status",
...
function showRep(type){
  ...
  if(type==='accountant'){
    ...
    if(!goalBuckets.length){
      ...
      currentReportExport = { title: r.title+' — '+y, year: y, reportType: r.title, headers:[t('rep_export_goal'),t('rep_export_saved'),t('rep_export_target'),t('rep_export_status')], rows: exportRows, numericCols:[1,2] };
    }
  } else {
    document.getElementById('rep-content').innerHTML = r.rows.map(([l,v])=>'<div class="rep-row"><span>'+l+'</span><span>'+v+'</span></div>').join('');
    currentReportExport = { title: r.title+' — '+y, year: y, reportType: r.title, headers:[t('rep_export_field'),t('rep_export_value')], rows: r.rows.map(([l,v])=>[l,v]), numericCols: [] };
  }
...
async function exportReportExcel(){
  ...
  track('report_exported',{format:'xlsx'});
}
function markBucketSpent(id){
  ...
  save();
  track('bucket_marked_spent',{name:b.name});
  showRep('goals');
}
...
function renderProgressCard(){
  ...
  if(isPremiumUnlocked()){
    ...
    el.innerHTML =
      '<div style="margin:0 14px 6px;position:relative;border-radius:var(--r);overflow:hidden">'+
...
// ===== PREMIUM GATING =====
// Single source of truth for whether premium features are unlocked. For now this only reads a
// local dev/testing toggle (Settings > Developer / Testing) — no real payment or account is involved.
// Any future premium-gated feature should call this function rather than checking state directly,
// so swapping in real Stripe/Supabase-backed entitlement later only requires changing this one place.
function isPremiumUnlocked(){
  return !!S.devPremiumUnlocked;
}
function saveDevPremiumToggle(){
  S.devPremiumUnlocked = document.getElementById('dev-premium-tog')?.checked || false;
  save();
  track('dev_premium_toggle',{on:S.devPremiumUnlocked});
  updateDash();
}
...
function renderSettings(){
  ...
  const devTog=document.getElementById('dev-premium-tog'); if(devTog) devTog.checked=!!S.devPremiumUnlocked;
}
